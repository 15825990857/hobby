...
(function (window, document, $) {
    ...
    var blackList = {
        queryIp: function () {
            var opts = $.extend(true, {},$.po('webuiPopover'), $('.ip-msg', 'able.dataTable').data(),{
                trigger: 'click',
                title: '详细信息',
                type: 'async',
            ...
            $('.ip-msg', 'table.dataTable ').webuiPopover(opts);
        },
        run: function () {
            ...
            $(document).one('hide.bs.modal', '#add', function () { // 模态窗隐藏后
                $blackList.formValidation('resetForm', true);
                $blackList.find('textarea').val('');
            });

            $(document).on('click', '.delete-tr', function (e) { // 删除当前行
                e.preventDefault();
